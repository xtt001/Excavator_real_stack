...
```text
gate_v2_predeclared_before_rerun
```
...
records the Gate audit, retired operands, and the predeclared v2 replacement.
The v2 rerun uses 1024 source-episode replicates and writes a new immutable
`sim_observable_cycle_v2` package. M1 remains unauthorized until that complete
package passes every M0 Gate.
...
- balanced top-1 accuracy at numeric anchors is an identifiability Gate against
  an episode-mapping permutation null, while event acceptance remains an
  own-prototype support/change-envelope check;
...
- a shared ready gap is selected once and becomes both `ready_end(i)` and
  `ready_start(i+1)`;
- the numeric observable anchor owns the deterministic representative row;
- the visually selected argmin/argmax row and offset remain diagnostics;
- confirmed numeric anchors rebuild `source_steps`, after which the complete
...
Interval-confirmation reliability is evaluated over the complete
source-episode outer bootstrap. The calibration rule chooses the maximum
cycle-minimum confirmation frequency that preserves all 9 current-to-next
transitions in both train and validation. Its one-sided 95% Wilson lower bound
must remain strictly above majority `0.5`. This threshold is frozen and
applied to each base-confirmed interval. Exact visual-point reselection,
signed-offset endpoint width, and wrong-prototype own-support coverage remain
diagnostics, not Gate operands. The formal v2 Gate additionally requires every
requested refit to complete and the eye/stick balanced-accuracy lower bound to
exceed its episode-mapping permutation-null p95.
...
After interval reliability is calibrated, the interval-confirmation mask is
frozen and reapplied to every retained event in each selector-successful
replicate.
...
`annotation_event_selected_sector_bootstrap_v2.json` records the masked
distribution and hashes both the selector core and the interval-selection/
confirmation-mask artifact. That masked distribution is the operand used by
the sector Gate. Numeric and sector boundary CIs must remain strictly between
their adjacent cluster-center CIs. The previous `CI width < 25% of cluster
gap` rule is diagnostic only. Numeric-candidate threshold bootstrap remains
the independent preceding Gate.
...
sim_observable_cycle_v2/
  ...
  annotation_thresholds_v2.json
  annotation_feature_input_manifest_v1.json
  annotation_feature_prototypes_v2.npz
  annotation_event_selector_prototypes_v2.npz
  annotation_event_selector_v2.json
  annotation_event_selected_sector_bootstrap_v2.json
  annotation_event_selector_gate_report_v2.json
  annotation_event_selections_pre_gate_v2.json
  annotation_event_selections_v2.json
  annotation_bootstrap_gate_report.json
  sector_visual_gate_report_v2.json
  annotation_manifest.json
  cycle_annotations.jsonl
  review_queue.jsonl
  split_groups.json
  transition_inventory.json
  transition_support_gate_report_v2.json
  condition_support_index.json
  resample_20hz_qc.json
  resample_20hz_gate_report_v2.json

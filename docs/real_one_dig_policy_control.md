...
本文档记录旧单 FPV one-dig policy 链路。当前 GMSL 四路默认现场入口使用
`testbed/testbed/configs/policy_real_gmsl_four_camera_v1.yaml` 和
`policy_bundles/real_gmsl_four_camera_v1/`。
...
- 默认运行 `shadow_zero`：旧模型会读取真实 `fpv + qpos/qvel` 并预测 action，但返回给 backend 的动作是 `[0, 0, 0, 0]`。
...
当前 GMSL 四路现场 preflight 请使用 `./scripts/run_policy_shadow_check.sh`。
